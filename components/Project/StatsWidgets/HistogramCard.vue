<template>
  <div class="histogram-card">
    ...
    <div v-if="hasData" class="histogram-wrap">
      ...
      <div class="x-axis-ticks" :class="{ rotated: rotateXLabels }">
        <span
          v-for="(label, index) in xTickLabels"
          :key="`x-tick-${index}`"
          class="x-tick"
        ><span class="x-tick-label">{{ label }}</span></span>
      </div>
      <div class="x-axis-title">{{ xAxisTitle }}</div>
    </div>
    <div v-else class="histogram-wrap-empty">No data</div>
    <div v-if="hasData && footerItems.length" class="widget-footer">
      <span
        v-for="(item, index) in footerItems"
        :key="item.label"
        class="footer-item"
      >
        <span v-if="index > 0" class="footer-sep">·</span>
        {{ item.label }} <span class="footer-value">{{ item.value }}</span>
      </span>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  xTickLabels: { type: Array, required: true },
  xAxisTitle: { type: String, required: true },
  // 5-SENSE has many narrow bins, so its labels are rotated to avoid overlap.
  rotateXLabels: { type: Boolean, default: false },
  // Footer summary chips, e.g. [{ label: 'Median', value: '12' }, ...].
  footerItems: { type: Array, default: () => [] },
})

const hasData = computed(
  () => props.binCounts.length > 0 && props.binCounts.some((count) => count > 0),
)
...
.x-axis-ticks.rotated .x-tick-label {
  position: absolute;
  top: 8px;
  right: 50%;
  padding-top: 0;
